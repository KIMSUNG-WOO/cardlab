...
export interface CardTheme {
  pageBg:string; heroBg:string; useAfgBg?:boolean
...
export function CardBase({ card, theme }: { card:BusinessCard; theme:CardTheme }) {
  ...
  // v4 design_options 마이그레이션
  const rawDesign = card.design_options ?? {}
  const design: CardDesignOptions = {
    ...DEFAULT_DESIGN_OPTIONS,
    ...rawDesign,
    icon_size:       typeof (rawDesign as any).icon_size === 'number' ? (rawDesign as any).icon_size : 22,
    font_size_name:  (rawDesign as any).font_size_name  ?? 28,
    font_size_sub:   (rawDesign as any).font_size_sub   ?? 14,
    font_size_body:  (rawDesign as any).font_size_body  ?? 13,
    animation_delay: (rawDesign as any).animation_delay ?? 0,
  }
  ...
  const delay = design.animation_delay ? `${design.animation_delay}s` : undefined
  ...
  const cl = design.contact_labels ?? {}
  ...
  return (
    <div
      ...
      style={{ minHeight:'100dvh', maxWidth:480, margin:'0 auto', background:t.pageBg, overflowY:'auto', overflowX:'hidden', position:'relative', animationDelay: delay }}
    >
      ...
      <div
        className="hero-anim"
        style={{ position:'relative', width:'100%', overflow:'hidden', background: t.useAfgBg ? 'transparent' : t.heroBg, height: heroCollapsed?'42vw':'88vw', maxHeight: heroCollapsed?'200px':'520px', minHeight: heroCollapsed?'130px':'260px', transition:'height 0.55s cubic-bezier(0.22,1,0.36,1), max-height 0.55s cubic-bezier(0.22,1,0.36,1)' }}
      >
        {t.useAfgBg && (
        ...
        <div className="fade-up-1" style={{ marginBottom:20 }}>
          {card.company_logo_url && (
            <div style={{ marginBottom:10 }}>
              <img src={card.company_logo_url} alt={card.company_name} style={{ height:28, objectFit:'contain', filter: (t.pageBg==='#ffffff'||t.pageBg==='#faf9f7'||t.pageBg==='#f5f6f8') ? 'none' : 'brightness(0) invert(1)' }} />
            </div>
          )}
          <h1 style={{ fontSize:fz.name, fontWeight:700, color:t.textName, marginBottom:4, letterSpacing:'-0.025em', lineHeight:1.2 }}>{card.name}</h1>
          {card.english_name && <p style={{ fontSize:fz.sub-1, color:t.accent, marginBottom:6, fontWeight:500 }}>{card.english_name}</p>}
        ...
        {card.phone && (
          <div className="fade-up-4" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:10 }}>
            <a href={`tel:${card.phone}`} className="touch-btn"
              style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, height:btnH, background:t.btnPrimary, color:t.btnPrimaryText, borderRadius:btnRadius, textDecoration:'none', fontSize:fz.body+1, fontWeight:700, boxShadow:`0 4px 20px ${t.accent}44` }}>
              {cl.phone ? `${cl.phone} ${card.phone}` : '📞 전화 문의하기'}
            </a>
            <a href={`sms:${card.phone}`} className="touch-btn"
              style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, height:btnH, background:t.btnSecondary, color:t.btnSecondaryText, borderRadius:btnRadius, textDecoration:'none', fontSize:fz.body+1, fontWeight:600, border:`1px solid ${t.btnSecondaryBorder}` }}>
              💬 SMS 문의
            </a>
          </div>
        )}
            ...
            {extraLinks.filter(l => l.url).map(link => (
              ...
              >
                <span style={{ display:'flex', alignItems:'center', gap:12 }}>
                  {design.show_icon && <span style={{ fontSize:iconSz }}>{link.emoji}</span>}
        ...
        <div style={{ padding:20, background:t.footerBg, border:`1px solid ${t.cardBorder}`, borderRadius:18 }}>
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
            {card.company_logo_url ? (
              ...
              <div style={{ width:36, height:36, borderRadius:8, background:t.cardBg, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, border:`1px solid ${t.cardBorder}`, fontSize:18 }}>🏢</div>
            )}
            <p style={{ fontSize:fz.body+2, fontWeight:700, color:t.textName, margin:0 }}>{card.company_name}</p>
          </div>
          <div style={{ paddingTop:12, borderTop:`1px solid ${t.cardBorder}`, display:'flex', flexDirection:'column', gap:8 }}>
            {card.address     && <p style={{ fontSize:12, color:t.textMuted, margin:0 }}>{cl.address ?? '📍'} {card.address}</p>}
            {card.website_url && <p style={{ fontSize:12, color:t.textMuted, margin:0 }}>{cl.website ?? '🌐'} {card.website_url.replace(/^https?:\/\//,'')}</p>}
            {card.phone       && <p style={{ fontSize:12, color:t.textMuted, margin:0 }}>{cl.phone ?? '📞'} {card.phone}</p>}
            {card.email       && <p style={{ fontSize:12, color:t.textMuted, margin:0 }}>{cl.email ?? '✉️'} {card.email}</p>}
          </div>
        </div>
