...
export function ImageUploader({ value, onChange, cardSlug, bucket = 'card-images', folder = 'profiles' }: Props) {
  ...
  function onDown(e: React.MouseEvent) {
    const p = getPct(e); const { x: cx, y: cy, w: cw, h: ch } = crop; const TH = 5
    const corners: [DragType, number, number][] = [['tl', cx, cy], ['tr', cx + cw, cy], ['bl', cx, cy + ch], ['br', cx + cw, cy + ch]]
    for (const [n, hx, hy] of corners) {
      if (Math.abs(p.x - hx) < TH && Math.abs(p.y - hy) < TH) { dragRef.current = { type: n, sx: p.x, sy: p.y, sc: { ...crop } }; return }
    }
  ...
  async function doUpload() {
    ...
    try {
      ...
      const dw = img.naturalWidth * sc, dh = img.naturalHeight * sc
      const dx = (W - dw) / 2, dy = (H - dh) / 2
      const cropXpx = ((crop.x / 100) * W - dx) / sc
      const cropYpx = ((crop.y / 100) * H - dy) / sc
      const cropWpx = (crop.w / 100) * W / sc
      const cropHpx = (crop.h / 100) * H / sc
      ...
      const blob = await new Promise<Blob>((res, rej) =>
        hc.toBlob(b => b ? res(b) : rej(new Error('변환 실패')), 'image/jpeg', 0.92)
      )

      // ⭐ 핵심 수정: 서버 API route 통해 업로드 (쿠키 세션 자동 인증)
    ...
    <div>
      ...
      {value && !showCrop && (
        <div style={{ marginBottom: 12, display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ position: 'relative', width: 88, height: 116, borderRadius: 10, overflow: 'hidden', border: '2px solid #dee2e6', flexShrink: 0 }}>
            <img src={value} alt="업로드 이미지" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'top' }} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <p style={{ fontSize: 12, color: '#6c757d', margin: 0 }}>업로드된 이미지</p>
      ...
      {!showCrop && !value && (
        <button type="button" onClick={() => fileRef.current?.click()}
          style={{ padding: '16px', background: '#f8f9fa', border: '2px dashed #ced4da', borderRadius: 12, color: '#868e96', fontSize: 13, cursor: 'pointer', width: '100%', textAlign: 'center' }}>
          📷 사진 클릭하여 업로드 (최대 10MB)
        </button>
      )}
      ...
      {showCrop && rawPreview && (
        <div style={{ background: '#fff', border: '1.5px solid #dee2e6', borderRadius: 16, padding: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={doUpload} disabled={uploading || !imgReady}
              style={{ flex: 1, padding: '12px', background: uploading ? '#adb5bd' : 'linear-gradient(135deg,#4263eb,#3b5bdb)', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: uploading || !imgReady ? 'not-allowed' : 'pointer' }}>
              {uploading ? '⏳ 업로드 중...' : !imgReady ? '로딩 중...' : '✓ 이 영역으로 저장'}
            </button>
            <button type="button" onClick={() => { setShowCrop(false); setRawPreview(null); setCropFile(null); setImgReady(false) }}
              style={{ padding: '12px 16px', background: '#f1f3f5', color: '#495057', border: '1px solid #dee2e6', borderRadius: 10, fontSize: 13, cursor: 'pointer' }}>취소</button>
          </div>
        </div>
      )}
