...
export function ImageUploader({ value, onChange, cardSlug, bucket = 'card-images', folder = 'profiles' }: Props) {
  ...
  const canvasSizeRef = useRef({ w: 380, h: 418 })
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(entries => {
      ...
      if (entry) {
        const w = Math.round(entry.contentRect.width) || 380
        canvasSizeRef.current = { w, h: Math.round(w * 1.1) }
        if (naturalImg.current && imgReady) draw(naturalImg.current)
      }
    })
    observer.observe(el)
  ...
  function onDown(e: React.MouseEvent) {
    const p = getPct(e); const { x: cx, y: cy, w: cw, h: ch } = crop; const HIT = 5
    const corners: [DragType, number, number][] = [['tl', cx, cy], ['tr', cx + cw, cy], ['bl', cx, cy + ch], ['br', cx + cw, cy + ch]]
    for (const [n, hx, hy] of corners) {
      if (Math.abs(p.x - hx) < HIT && Math.abs(p.y - hy) < HIT) {
        dragRef.current = { type: n, sx: p.x, sy: p.y, sc: { ...crop } }; return
      }
    }
    if (p.x > cx && p.x < cx + cw && p.y > cy && p.y < cy + ch)
      dragRef.current = { type: 'move', sx: p.x, sy: p.y, sc: { ...crop } }
  }
  ...
  async function doUpload() {
    ...
    try {
      ...
      const outW = Math.max(1, Math.min(Math.round((crop.w / 100) * W / sc), img.naturalWidth))
      const outH = Math.max(1, Math.min(Math.round((crop.h / 100) * H / sc), img.naturalHeight))
    ...
    <div>
      ...
      {showCrop && rawPreview && (
        <div style={{ background: '#fff', border: '1.5px solid #dee2e6', borderRadius: 16, padding: 18 }}>
          ...
          <div ref={containerRef} style={{ position: 'relative', width: '100%', marginBottom: 14, borderRadius: 10, overflow: 'hidden', userSelect: 'none', background: '#000' }}>
            <canvas ref={previewCanvas} style={{ display: 'block', width: '100%', height: 'auto', touchAction: 'none', cursor: 'crosshair' }}
